...
import { Search, Brain, Plus, Settings, History, Info, Sparkles } from "lucide-react";
    ...
    <div className="flex h-screen w-full overflow-hidden text-slate-50">
      ...
      <aside className="w-20 lg:w-64 border-r border-white/10 flex flex-col items-center lg:items-start py-8 px-4 glass">
        <div className="flex items-center gap-3 mb-12 px-2">
          ...
          <span className="hidden lg:block font-bold text-xl tracking-tight">AI Memory</span>
        </div>
        ...
        <section className="flex-1 overflow-y-auto p-8 space-y-8 z-10 custom-scrollbar">
          ...
          >
            <form onSubmit={handleIngest} className="space-y-4">
              <div className="flex items-center gap-2 text-cyan-400 mb-2">
                <Sparkles size={16} />
                <span className="text-xs font-bold uppercase tracking-widest">Thought Ingestion</span>
              </div>
              <textarea 
          ...
          <div className="max-w-4xl mx-auto space-y-6">
            <h2 className="text-lg font-semibold flex items-center gap-2 px-2">
              Recent Memories 
              <span className="text-xs font-normal text-slate-500 bg-white/5 px-2 py-0.5 rounded-full">{memories.length}</span>
            </h2>
              ...
              {memories.length > 0 ? (
                memories.map((m, i) => (
                  ...
                  >
                    ...
                    <div className="mt-4 flex items-center gap-4 opacity-0 group-hover:opacity-100 transition-opacity">
                       <button className="text-xs text-slate-400 hover:text-cyan-400">View Details</button>
                       <button className="text-xs text-slate-400 hover:text-red-400">Forget</button>
                    </div>
                  </motion.div>
                ))
              ) : (
                <div className="py-20 flex flex-col items-center text-center space-y-4">
                  ...
                  <div>
                    ...
                    <p className="text-sm text-slate-500">Start by adding a thought above.</p>
                  </div>
                </div>
              )}
            </div>
...
function NavItem({ icon, label, active = false }: { icon: any, label: string, active?: boolean }) {
  return (
    <div className={cn(
      ...
      active ? "bg-cyan-500/10 text-cyan-400 border border-cyan-400/20 shadow-[0_0_15px_rgba(34,211,238,0.1)]" : "text-slate-400 hover:bg-white/5 hover:text-slate-200"
    )}>
      {icon}
...
function Tag({ label, color }: { label: string, color: 'cyan' | 'indigo' }) {
  ...
  return (
    <span className={cn("px-2 py-0.5 rounded-lg text-[10px] font-bold border", colors[color])}>
      {label}
    </span>
  );
}
